...
const (
	systemd = "systemd-amd64-239-10"
	bash    = "bash-amd64-4.4.18-3"
)
...
func installHTTPMultipleVersions(ctx context.Context, tmpdir string, pkg ...string) error {
	...
	// dependencies):
	addrs := make(map[string]string) // pkg → addr
	for _, pkg := range []string{"systemd-amd64-239-10", "systemd-amd64-100"} {
		rtmpdir, err := ioutil.TempDir("", "distritest")
		if err != nil {
			return err
		}
		...
		for _, suffix := range []string{"squashfs", "meta.textproto"} {
			if err := os.Rename(
				filepath.Join(rtmpdir, "pkg", "systemd-amd64-239-10."+suffix),
